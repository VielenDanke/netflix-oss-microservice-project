...
curl -XPUT -u elastic:changeme 'elasticsearch:9200/_xpack/security/user/elastic/_password?pretty' -H 'Content-Type: application/json' -d'
{
  "password": "${ES_PASSWORD}"
}
'

curl -u elastic 'elasticsearch:9200/_xpack/security/_authenticate?pretty'

curl -XPUT -u elastic '127.0.0.1:9200/_xpack/security/user/kibana/_password?pretty' -H 'Content-Type: application/json' -d'
{
  "password": "${ES_PASSWORD}"
}
'
